...
V BTree<K, V>::find(const BTreeNode* subroot, const K& key) const
{
    ...
    if (!subroot -> elements.empty() && subroot -> elements.size() > first_larger_idx && subroot -> elements[first_larger_idx].key == key) {
...
void BTree<K, V>::insert(BTreeNode* subroot, const DataPair& pair)
{
    ...
    /* TODO Your code goes here! */
     if (!subroot -> elements.empty() && subroot -> elements.size() > first_larger_idx && subroot -> elements[first_larger_idx].key == pair.key) {
        return;
    } else if (subroot -> is_leaf) {
